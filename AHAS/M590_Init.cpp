...
bool M590::init(unsigned long baudRate, HardwareSerial *gsmSerial,char pinOnOff, char* codePIN)
{
    bool retVal = false;
    ...
    while ((millis() < startTime + INIT_TIMEOUT) && (continueInit == true))
    {
        switch (_initState)
        {
            ...
            case M590_INIT_CHECK_ALIVE:
                ...
                {
                    printDebug(M590_ERROR_TOGGLE_ONOFF);
                    _initState = M590_INIT_TOGGLE_ONOFF;
                }
                */

                break;

            case M590_INIT_TOGGLE_ONOFF:

                digitalWrite(pinOnOff, HIGH);
                delay(500); /* 500ms impulse*/
                digitalWrite(pinOnOff, LOW);

                delay(25000); /* Temporary delay before checking the alive state */
